...
def ExtractFlowData(path='../4G/1月'):
    ...
    # 读取已录入数据的日期
    exist_date = list(read_process_date())
    print('------------------------------------------------------')
    print(f'读取到已经录入的信息为: {exist_date}')
    print('------------------------------------------------------')
...
def extract_city_data(city_name):
    ...
    sql = f"""
            ...
            FROM {table_name} a
            JOIN (SELECT * FROM {table_name2} WHERE city = "{city_name}") b
            ON a.cgi = b.CGI
            """
    df = pd.read_sql(sql, conn)
    conn.close()
    df.to_csv('../4Gprocessed_data/' + city_name + '.csv')
