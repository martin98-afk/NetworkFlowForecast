    ...
    def __init__(self, embed_dim, feat_dim, num_heads, ff_dim, rate=0.1):
        ...
        self.ffn = keras.Sequential(
            [
                ...
                layers.Conv1D(filters=feat_dim, kernel_size=1)
            ]
        )
...
class Transfomer():
    ...
    def nn_sctructure(self, N_INPUTS, N_FEATURES, N_OUTPUTS):
        ...
        x = layers.MultiHeadAttention(num_heads=N_HEADS, key_dim=EMBED_DIM)(x, x) + x
        x = layers.LayerNormalization(epsilon=1e-6)(x)

        x = keras.Sequential([
            # layers.Dense(FF_DIM, activation="gelu"),
            # layers.Dense(N_OUTPUTS*2, activation="gelu")
            layers.Conv1D(filters=FF_DIM, kernel_size=1, activation="relu"),
            layers.Conv1D(filters=N_OUTPUTS * 2, kernel_size=1)
        ])(x)
